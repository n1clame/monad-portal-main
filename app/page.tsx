...
const dict: Record<Lang, any> = {
  ...
  ru: {
    ...
    faq: {
      title: "FAQ",
      hint: "Наведи, чтобы увидеть ответ",
      items: [
        { q: "Monad совместим с EVM?", a: "Да — контракты и инструменты переносятся с минимальными изменениями." },
        { q: "Как вступить в тестнет?", a: "Добавь тестнет, получи MON из крана, попробуй dApps и поделись фидбеком в Discord." },
        { q: "Какие комиссии?", a: "Низкие комиссии благодаря оптимизированному и параллельному исполнению." },
        { q: "Где смотреть транзакции?", a: "В тестнет-эксплорере по ссылке в футере." },
        { q: "Есть ли параллельное исполнение?", a: "Да, это ключ к производительности." },
        { q: "Дружит с привычными тулзами?", a: "MetaMask, Hardhat, Foundry — всё ок." },
        { q: "Как быстро добавить тестнет?", a: "Нажми кнопку ‘Добавить тестнет Monad’ на первом экране." },
        { q: "Где спросить о помощи?", a: "В Discord — есть каналы для девов и комьюнити." },
        { q: "Можно показывать NFT?", a: "Страница NFT убрана по просьбе; фокус на Discord и доках." },
        { q: "Где читать доки и вайтпейпер?", a: "На docs.monad.xyz есть и то, и другое." },
      ],
    },
  ...
  uk: {
    ...
    faq: {
      title: "FAQ",
      hint: "Наведи, щоб побачити відповідь",
      items: [
        { q: "Monad сумісний з EVM?", a: "Так — контракти й інструменти переносяться з мінімальними змінами." },
        { q: "Як приєднатися до тестнету?", a: "Додай тестнет, отримай MON з фаусету, спробуй dApps і поділися фідбеком у Discord." },
        { q: "Які комісії?", a: "Низькі комісії завдяки оптимізованому та паралельному виконанню." },
        { q: "Де відстежувати транзакції?", a: "У тестнет-експлорері за посиланням у футері." },
        { q: "Чи є паралельне виконання?", a: "Так, це ключова частина продуктивності." },
        { q: "Чи зручно з тулінгом EVM?", a: "MetaMask, Hardhat, Foundry — усе готово." },
        { q: "Як швидко додати тестнет?", a: "Скористайся кнопкою ‘Додати тестнет Monad’ на першому екрані." },
        { q: "Де питати поради?", a: "У Discord — є канали для девів і спільноти." },
        { q: "Можу показувати NFT?", a: "Сторінку NFT прибрано на прохання; фокус на Discord і доках." },
        { q: "Де читати доки/вайтпейпер?", a: "docs.monad.xyz містить обидва." },
      ],
    },
...
// универсальный компонент: кладёт ПОСЛЕДНЕЕ слово на вторую строку
const TwoLine = ({ text }: { text: string }) => {
  const parts = text.trim().split(/\s+/);
  return (
    <span className="leading-tight text-center">
      <span className="block">{parts.slice(0, -1).join(" ")}</span>
...
function StickyHero({ t }: any) {
  ...
  const Chip = ({ text }: { text: string }) => (
    <div className="rounded-2xl border border-white/20 bg-white/5 px-4 py-3 text-sm backdrop-blur transition-all duration-200 hover:shadow-[0_0_20px_rgba(167,139,250,0.35)] hover:scale-[1.02] flex items-center justify-center text-center min-h-[58px]">
      ...
      <div ref={ref} className="grid lg:grid-cols-2 gap-10 items-center">
        <motion.div style={{ y }}>
          ...
          <div className="pixel-cta-group mt-6 flex flex-wrap gap-3">
            <Button className="rounded-2xl" onClick={() => document.getElementById("start")?.scrollIntoView({ behavior: "smooth" })}>
              {t.hero.start}
            </Button>
            <Button variant="secondary" className="rounded-2xl" onClick={() => (window as any).ethereum?.request?.({ method: "wallet_addEthereumChain", params: [MONAD_NET.params] })}>
              {t.hero.addNet}
            </Button>
            <Button variant="secondary" className="rounded-2xl" onClick={() => (location.href = "/discord")}>
              <span className="grad-text">{t.hero.aboutDiscord}</span>
            </Button>
          </div>
          <div className="mt-10 grid grid-cols-2 sm:grid-cols-4 gap-4 items-stretch">
  ...
  return (
    <Card className="bg-white/5 border-white/10 h-full transition-all duration-300 hover:bg-gradient-to-r hover:from-violet-700/25 hover:to-fuchsia-700/25 hover:shadow-[0_0_24px_rgba(167,139,250,0.45)] group">
      <CardContent className="p-5 h-full">
        <div className="flex flex-col items-center justify-center text-center gap-3 h-full min-h-[180px]">
          <div className="p-2 rounded-xl bg-violet-500/15 flex items-center justify-center">
            <Icon className="w-5 h-5" />
          </div>
          <div>
            <div className="font-mono transition-transform duration-200 group-hover:scale-[1.03]">{title}</div>
            <div className="text-sm text-white/75 font-sans mt-1 leading-snug max-w-[28ch]">{desc}</div>
          </div>
        </div>
      </CardContent>
    </Card>
    ...
    <div className="min-h-screen bg-violet-950 text-white selection:bg-violet-500/30">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center pixel-nav ml-8">
            {[
              ...
              <a key={n.id} href={n.href} className={`text-xs px-1 py-0.5 text-white/80 hover:text-white transition ${n.id === "keone" ? "rainbow-text" : ""}`}>
                {n.label}
              </a>
            ))}
            <Button variant="secondary" className="rounded-xl px-2 py-1 text-xs h-7" onClick={cycleLang}>
              {t.switcher[lang]}
            </Button>
          </nav>
        ...
        <div className="relative">
          ...
          <div className="relative max-w-2xl mx-auto">
            <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur p-8 text-center">
              ...
              <div className="mt-2 text-4xl pixel gasShimmer animate-gas-wiggle inline-block px-3 py-1 rounded">0.00021 MON</div>
      ...
      <SectionWrap id="start">
        <div className="pixel-text-base pixel-start max-w-4xl mx-auto">
          ...
          <div className="relative mt-8 md:mt-12">
            {/* enlarge oval and place below grid */}
            <div className="absolute -inset-8 md:-inset-10 rounded-[9999px] border-2 border-violet-300/40 pointer-events-none z-0"></div>

            <div className="relative z-10 grid md:grid-cols-4 gap-4 py-4">
              {t.start.steps.map((s: any, i: number) => (
                <Card
                  key={i}
                  className="bg-white/5 border-white/10 transition-all duration-300 hover:bg-white/10 hover:shadow-[0_0_24px_rgba(167,139,250,0.35)] overflow-hidden min-w-0"
                >
                  {/* center text; button pinned at bottom but not outside */}
                  <CardContent className="p-5 flex flex-col min-h-[236px] min-w-0">
                    <div className="flex-1 flex flex-col items-center justify-center text-center min-w-0">
                      <h3 className="font-semibold font-mono leading-tight max-w-[18ch] mx-auto">{s.t}</h3>
                      <p className="text-sm text-white/75 mt-2 font-sans leading-snug max-w-[22ch] mx-auto">
                        {s.link ? (
                          <a className="underline decoration-violet-400 hover:text-white" href={s.link} target="_blank">
                            {s.d}
                          </a>
                        ) : (
                          s.d
                    ...
                    {i === 1 && (
                      <Button
                        className="mt-4 w-full max-w-full rounded-2xl tracking-normal overflow-hidden text-center whitespace-normal break-words px-3 py-2 h-auto min-h-[44px] justify-center"
                        onClick={() =>
                    ...
                    {i === 2 && (
                      ...
                      >
                        <TwoLine text={t.start.faucetBtn} />
                      </Button>
                    )}
                  </CardContent>
                </Card>
      ...
      <SectionWrap id="faq">
        <div className="pixel-text-base pixel-faq">
          <h2 className="text-3xl font-bold font-serif">{t.faq.title}</h2>
          <p className="text-white/70 mt-2">{t.faq.hint}</p>
          <div className="mt-6">
            <HoverFaq items={t.faq.items} />
          </div>
        </div>
      </SectionWrap>
        ...
        <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-white/70">
            ...
            <div className="flex items-center gap-4">
              <a href={LINKS.docs} target="_blank" rel="noreferrer" className="hover:text-white">
                Docs
              </a>
